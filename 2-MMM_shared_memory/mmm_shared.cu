...
//MMM kernel
__global__ void kernel_mmm_shared(int width, float* M, float* N, float* P){
  // TILE_WIDTH = blockDim.x; // Square blocks are used
  const int TILE_WIDTH = 16; // Set to a fixed value because the shared memory allocation 
                             // in lines 35, and 36 need the size to be known and compile time.
  ...
  for(int m=0; m<width/TILE_WIDTH; m++){
    ...
    for(int k=0; k<TILE_WIDTH; k++){
      Pvalue += Mds[ty_id][k]*Nds[k][tx_id];
    }
...
int main(int argc, char **argv){
  ...
  ********************/
  ...
  // Launch the kernel
  kernel_mmm_shared<<<dimGrid, dimBlock>>>(WIDTH, d_M, d_N, d_P);
